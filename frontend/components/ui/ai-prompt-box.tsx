...
// Typed motion helpers to sidestep noisy className inference issues from framer-motion typings
// Cast to any to keep JSX props like className happy in TS
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const MotionDiv = motion.div as any;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const MotionSpan = motion.span as any;

// Radix + utility types for speech recognition
// Minimal cn helper for merging class names
...
const styles = `
  *:focus-visible {
  ...
  .light textarea::-webkit-scrollbar-thumb:hover {
    background-color: hsl(var(--muted));
  }
`;
...
>(({ className, sideOffset = 4, ...props }, ref) => (
  <TooltipPrimitive.Content
    ...
    className={cn(
      'z-50 overflow-hidden rounded-md border border-purple-500/50 bg-neutral-900 px-3 py-1.5 text-sm text-white shadow-md animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2',
      className,
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed left-[50%] top-[50%] z-50 grid w-full max-w-[90vw] md:max-w-[800px] translate-x-[-50%] translate-y-[-50%] gap-4 border border-purple-500/50 bg-neutral-900 p-0 shadow-xl duration-300 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 rounded-2xl',
        className,
      )}
      {...props}
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 z-10 rounded-full bg-neutral-800/80 p-2 hover:bg-neutral-800 transition-all">
        <X className="h-5 w-5 text-white hover:text-purple-400" />
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({ className, variant = 'default', size = 'default', ...props }, ref) => {
  const variantClasses = {
    ...
    outline: 'border border-purple-500/50 bg-transparent hover:bg-neutral-800 text-white',
...
const VoiceRecorder: React.FC<VoiceRecorderProps> = ({ isRecording, onStartRecording, onStopRecording, visualizerBars = 32 }) => {
  ...
  const prevRecording = React.useRef(false);
      ...
      <TooltipProvider>
        ...
        >
          <div
            ref={ref}
            className={cn(
              'rounded-3xl border bg-neutral-900 p-2 shadow-[0_8px_30px_rgba(107,47,216,0.24)] transition-all duration-300',
              isLoading ? 'border-purple-500/70' : 'border-purple-500/50',
...
const PromptInputAction: React.FC<PromptInputActionProps> = ({ tooltip, children, side = 'top', contentClassName, ...props }) => {
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  ...
  React.useEffect(() => {
    if (typeof document === 'undefined' || styleInjected.current) return;
  ...
  const handleSubmit = () => {
    ...
    const formattedInput = messagePrefix ? `${messagePrefix}${trimmed}]` : trimmed;
  ...
  const handleStopRecording = (duration: number) => {
    ...
    setIsRecording(false);
    if (!speechSupported) {
      const fallbackMessage = `[Voice message - ${duration} seconds]`;
  ...
  return (
    <>
      <PromptInput
        ...
        className={cn(
          'w-full bg-[hsl(var(--chat-input))] border-[hsl(var(--chat-input-border))] shadow-lg transition-all duration-300 ease-in-out',
      ...
      >
        ...
        {speechError && (
          <p className="px-2 pb-2 text-xs text-red-400">Speech input unavailable: {speechError}</p>
        )}
          ...
          >
            <PromptInputAction tooltip="Upload image">
              <button
                ...
                className="flex h-8 w-8 text-white/70 cursor-pointer items-center justify-center rounded-full transition-colors hover:bg-neutral-800 hover:text-purple-400"
            ...
            <div className="flex items-center">
              <button
                ...
                className={cn(
                  ...
                  showSearch ? 'bg-purple-600/20 border-purple-500 text-purple-400' : 'bg-transparent border-transparent text-white/70 hover:text-purple-400',
                )}
                disabled={disabled}
              >
                <div className="w-5 h-5 flex items-center justify-center flex-shrink-0">
                  ...
                  >
                    <Globe className={cn('w-4 h-4', showSearch ? 'text-purple-400' : 'text-inherit')} />
                  </MotionDiv>
                </div>
                <AnimatePresence>
                  {showSearch && (
              ...
              <button
                ...
                className={cn(
                  'rounded-full transition-all flex items-center gap-1 px-2 py-1 border h-8',
                  showThink ? 'bg-purple-600/20 border-purple-500 text-purple-400' : 'bg-transparent border-transparent text-white/70 hover:text-purple-400',
                )}
                disabled={disabled}
              >
                <div className="w-5 h-5 flex items-center justify-center flex-shrink-0">
              ...
              <button
                ...
                className={cn(
                  'rounded-full transition-all flex items-center gap-1 px-2 py-1 border h-8',
                  showCanvas ? 'bg-purple-600/20 border-purple-500 text-purple-400' : 'bg-transparent border-transparent text-white/70 hover:text-purple-400',
                )}
                disabled={disabled}
              >
                <div className="w-5 h-5 flex items-center justify-center flex-shrink-0">
                  <MotionDiv
                    ...
                    transition={{ type: 'spring', stiffness: 260, damping: 25 }}
                  >
                    <FolderCog className={cn('w-4 h-4', showCanvas ? 'text-purple-400' : 'text-inherit')} />
                  </MotionDiv>
                </div>
                <AnimatePresence>
                  {showCanvas && (
            ...
            <Button
              ...
              className={cn(
                ...
                isRecording
                  ...
                  : 'bg-transparent hover:bg-neutral-800 text-white/70 hover:text-purple-400',
              )}
              onClick={() => {
                if (isRecording) {
                  setIsRecording(false);
...
// Simple demo export for local testing
  ...
  return (
    <div className="flex w-full h-screen justify-center items-center bg-[radial-gradient(125%_125%_at_50%_101%,rgba(245,87,2,1)_10.5%,rgba(245,120,2,1)_16%,rgba(245,140,2,1)_17.5%,rgba(245,170,100,1)_25%,rgba(238,174,202,1)_40%,rgba(202,179,214,1)_65%,rgba(148,201,233,1)_100%)]">
      <div className="p-4 w-[500px]">
        <PromptInputBox onSend={(message, messageFiles) => console.log(message, messageFiles)} />
      </div>
    </div>
  );
};
